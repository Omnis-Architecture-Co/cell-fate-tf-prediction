#!/usr/bin/env python3
"""
VAL-DICT-001 v6b: Mitochondrial-Excluded Re-Validation
=======================================================
Same as v6 but excludes Mitochondrial from:
  1. Vocabulary tokens classified as Mitochondrial (removed from scoring)
  2. Ground truth labels (Mitochondrial removed from GT sets)
  3. Proteins whose ONLY GT label was Mitochondrial are excluded

This reveals the vocabulary's discrimination power across the remaining 31 departments
without the attractor effect masking real signal.

Outputs: VAL-DICT-001_v6b_noMito_*.json/md
"""
...
PREFIX = "VAL-DICT-001_v6b_noMito"
SEEDS = list(range(42, 52))
WORD_THRESHOLDS = [2, 5, 10, 20, 50]
CONF_THRESHOLD = 0.5
EXCLUDED_DEPT = "Mitochondrial"
...
def load_data():
    ...
    for r in vrows:
        ...
        vocab_full[h] = entry
        if r["primary_function"] != EXCLUDED_DEPT:
            vocab_no_mito[h] = entry

    print(f"  Vocab: {len(vocab_full):,} total, {len(vocab_no_mito):,} after excluding {EXCLUDED_DEPT}")
    mito_tokens = len(vocab_full) - len(vocab_no_mito)
    print(f"  Removed {mito_tokens:,} {EXCLUDED_DEPT} tokens")
    ...
    print(f"  {len(all_uids):,} human proteins, {len(gt):,} with non-Mito GT (conf>={CONF_THRESHOLD})")
    return human, vocab_full, vocab_no_mito, ptokens, all_uids, gmap, gt, canonical, mito_tokens
...
def make_plots(all_uids, ptokens, vset, vocab, gt, seed_results):
    try:
    ...
    for uid in all_uids:
        ...
        ok5 = bool({d for d, _ in p["ranked"][:5]} & td)
        data_t1.append((nw, 1 if ok1 else 0))
        data_t3.append((nw, 1 if ok3 else 0))
        data_t5.append((nw, 1 if ok5 else 0))
    ...
    ax.set_title("VALDICT001 Accuracy (Mitochondrial Excluded) — 31 Departments", fontsize=13)
    ...
    merged_f1 = defaultdict(lambda: {"precision": [], "recall": [], "f1": [], "support": 0})
    ...
    depts_sorted = sorted(merged_f1.keys(), key=lambda d: sum(merged_f1[d]["f1"])/len(merged_f1[d]["f1"]), reverse=True)
    fig, ax = plt.subplots(figsize=(14, 6))
    f1_means = [sum(merged_f1[d]["f1"])/len(merged_f1[d]["f1"]) for d in depts_sorted]
    colors = ["#16a34a" if f >= 0.3 else "#f59e0b" if f >= 0.15 else "#dc2626" for f in f1_means]
    ax.bar(range(len(depts_sorted)), f1_means, color=colors, alpha=0.8)
    ax.set_xticks(range(len(depts_sorted)))
    ax.set_xticklabels(depts_sorted, rotation=65, ha="right", fontsize=7)
    ax.set_ylabel("F1 Score (mean across 10 seeds)")
    ax.set_title("Per-Department F1 Scores (Mitochondrial Excluded)")
    ax.axhline(y=sum(f1_means)/len(f1_means), color="blue", ls="--", alpha=0.4, label=f"Mean F1: {sum(f1_means)/len(f1_means):.3f}")
    ...
    top_attractors = sorted(wrong_target.items(), key=lambda x: -x[1])[:5]
    print(f"\n  Top confusion attractors (post-Mito removal):")
    for d, c in top_attractors:
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("VAL-DICT-001 v6b: Mitochondrial-Excluded Validation")
    print("=" * 70)
    print(f"Started: {datetime.now(timezone.utc).isoformat()}\n")

    human, vocab_full, vocab_no_mito, ptokens, all_uids, gmap, gt, canonical, mito_tokens = load_data()
    vset = set(vocab_no_mito.keys())
    ...
    print(f"\n  {n_gt:,} proteins with non-Mito GT, {len(all_depts)} departments")
    ...
    preds_all = predict_topk(all_uids, ptokens, vset, vocab_no_mito, k=5)
    well_known = {}
    ...
    print(f"\n[3] Running {len(SEEDS)} held-out splits (no Mitochondrial)...")
    seed_results = []
    for seed in SEEDS:
        res = run_seed(seed, all_uids, ptokens, vset, vocab_no_mito, gt, df, mc, all_depts)
    ...
    print(f"\n  === CORE METRICS (Mitochondrial Excluded) ===")
    ...
    print(f"  {'Words':>6} | {'Top-1':>12} | {'Top-3':>12} | {'Top-5':>12} | {'Adj@3':>12} | {'Lift/F':>7} | {'n':>6}")
    for wt in WORD_THRESHOLDS:
        ...
        fr = ms([s["thresh_freq"].get(wt, s["overall_freq"]) if "thresh_freq" in s else s["overall_freq"] for s in seed_results]) if hasattr(seed_results[0], 'get') else (freq_m, 0)
        n_m = ms([s["topk_n_thresh"][wt] for s in seed_results])
        lift = t1[0]/freq_m if freq_m > 0 else 0
        print(f"  >={wt:>4} | {t1[0]:.4f}+/-{t1[1]:.4f} | {t3[0]:.4f}+/-{t3[1]:.4f} | {t5[0]:.4f}+/-{t5[1]:.4f} | {a3[0]:.4f}+/-{a3[1]:.4f} | {lift:.2f}x | ~{n_m[0]:.0f}")
    ...
    print(f"  Calibration ratio: {hm_m/lm_m:.2f}x" if lm_m > 0 else "")
    ...
    print(f"\n[5] Generating plots...")
    make_plots(all_uids, ptokens, vset, vocab_no_mito, gt, seed_results)

    print(f"\n[6] Comparison: v6 (with Mito) vs v6b (without Mito)")
    v6p = os.path.join(OUT_DIR, "VAL-DICT-001_v6_paper_results.json")
    if os.path.exists(v6p):
        with open(v6p) as f:
            v6 = json.load(f)
        print(f"  {'Metric':25s} {'v6 (with Mito)':>15s} {'v6b (no Mito)':>15s} {'Delta':>10s}")
        v6_t1 = v6["accuracy"]["top1"]["mean"]
        print(f"  {'Top-1':25s} {v6_t1:>15.4f} {ma:>15.4f} {ma-v6_t1:>+10.4f}")
        v6_t3 = v6["accuracy"]["top3"]["mean"]
        t3m = ms([s["topk"][3] for s in seed_results])[0]
        print(f"  {'Top-3':25s} {v6_t3:>15.4f} {t3m:>15.4f} {t3m-v6_t3:>+10.4f}")
        v6_r = v6["enrichment"]["pearson_r"]
        print(f"  {'Pearson r':25s} {v6_r:>15.4f} {mr:>15.4f} {mr-v6_r:>+10.4f}")
        v6_freq = v6["accuracy"]["freq_baseline"]
        print(f"  {'Freq baseline':25s} {v6_freq:>15.4f} {freq_m:>15.4f} {freq_m-v6_freq:>+10.4f}")
        v6_lift = v6_t1/v6_freq if v6_freq else 0
        lift_now = ma/freq_m if freq_m else 0
        print(f"  {'Lift over freq':25s} {v6_lift:>15.2f}x {lift_now:>15.2f}x")

    results = {
        "validation_id": "VAL-DICT-001", "version": "v6b_noMito",
        "timestamp": datetime.now(timezone.utc).isoformat(),
        "excluded": EXCLUDED_DEPT,
        "mito_tokens_removed": mito_tokens,
        "methodology": {"pipeline": "v2_6bit", "species": "Homo sapiens",
            "gt_join": "gene_name", "gt_confidence": f">={CONF_THRESHOLD}",
            "exclusion": f"{EXCLUDED_DEPT} removed from vocab predictions AND ground truth",
            "seeds": SEEDS},
    ...
    print(f"  r={mr:.4f}, Top-1={ma:.4f}, Top-3={ms([s['topk'][3] for s in seed_results])[0]:.4f}, Adj@3={ma3:.4f}")
    t3_50 = ms([s["topk_by_thresh"][50][3] for s in seed_results])[0]
    t5_50 = ms([s["topk_by_thresh"][50][5] for s in seed_results])[0]
    print(f"  >=50 words: Top-3={t3_50:.4f}, Top-5={t5_50:.4f}")
    print(f"  Confidence: high={hm_m:.4f} low={lm_m:.4f} ratio={hm_m/lm_m:.2f}x")
    print(f"{'='*70}")
